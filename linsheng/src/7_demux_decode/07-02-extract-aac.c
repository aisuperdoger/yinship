...
int main(int argc, char *argv[])
{
    ...
    AVFormatContext *ifmt_ctx = NULL;  // 为什么不用使用avformat_alloc_context();？
    AVPacket pkt;
    ...
    // 获取解码器信息
    if((ret = avformat_find_stream_info(ifmt_ctx, NULL)) < 0) // avformat_find_stream_info是否对ifmt_ctx做了某些操作？？
    {
        av_strerror(ret, errors, 1024);
