...
int main(int argc, char **argv)
{
    ...
    // 获取裸流的解析器 AVCodecParserContext(数据)
    ...
    // 分配codec上下文。上下文就是保存一些信息，比如此处的codec上下文，就是代表输入视频应该使用什么样的编解码器
    ...
    while (data_size > 0)
    {
        ...
        ret = av_parser_parse2(parser, codec_ctx, &pkt->data, &pkt->size,
                               data, data_size,                    // 将数据解析到&pkt->data和&pkt->size中
                               AV_NOPTS_VALUE, AV_NOPTS_VALUE, 0); // 0代表从起始位置读数据
                                                                   // av_parser_parse2没有为pkt->data分配内存，而是让pkt->data直接指向data所指向的内存
        if (ret < 0)
        {
