...
// nBufSize代表pBuf有多少个字节
// nOffset代表处理到pBuf的第几个字节
...
int CFlvParser::Parse(uint8_t *pBuf, int nBufSize, int &nUsedLen) // pBuf指向的是整个flv文件
{
    ...
    if (_pFlvHeader == 0) // 将flv文件中的flv header提取出来。_pFlvHeader == 0时，说明flv header还没有解析，flv header只解析一次。
    {
        CheckBuffer(9);     
...
CFlvParser::FlvHeader *CFlvParser::CreateFlvHeader(uint8_t *pBuf) // pBuf就相当于输入的flv文件
{
    ...
    pHeader->nHeadSize = ShowU32(pBuf + 5);         // 头部长度。ShowU32：读取32字节代表的多大的数
