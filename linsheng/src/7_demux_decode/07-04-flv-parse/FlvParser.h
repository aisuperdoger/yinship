...
private:
    ...
    // 8bit的char类型使用<<以后都会统一地提升为32bit的int类型，所以上面这样处理以后，相当于得到了pbuf指向的4个字节所代表的int值
    ...
    static void WriteU64(uint64_t & x, int length, int value)
    {
        ...
        x = (x << length) | ((uint64_t)value & mask); // x << length代表整体向左移动，从而给((uint64_t)value & mask)腾出右边的空间。
    }
